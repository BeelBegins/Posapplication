...
const posAPI = {
  ...
  syncItemCatalog:(mode?:"auto"|"full")=>core.syncItemCatalog(m=>catalogListeners.forEach(cb=>cb(m)),mode),syncBarcodesOnly:(mode?:"auto"|"full")=>core.syncBarcodesOnly(m=>catalogListeners.forEach(cb=>cb(m)),mode),resolveCatalogItem:(q:string)=>core.resolveCatalogItem(q),getCatalogTotals:async()=>db.getCatalogTotals(),syncFbrConfig:(mode?:"auto"|"full")=>core.syncFbrConfig(mode),getFbrSyncState:async()=>db.getFbrSyncState(),
