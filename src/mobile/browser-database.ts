...
function rebuildCatalogIndexes(): void {
  ...
  for (const row of state.barcodes) {
    const stored = rowText(row, "barcode");
    if (!stored) continue;
    // Index every GTIN padding variant so a scanner's EAN-13 leading-0 form
    // still resolves a UPC-A (or reverse) stored on the item.
    for (const variant of barcodeVariants(stored)) {
      const key = variant.toLowerCase();
      if (!barcodeIndex.has(key)) barcodeIndex.set(key, row);
    }
  }
...
function resolveBarcodeRow(query: string): Row | undefined {
  for (const variant of barcodeVariants(query)) {
    const row = barcodeIndex.get(variant.toLowerCase());
    if (row) return row;
  }
  return undefined;
}
...
export const mobileDatabase: IDatabaseService = {
  ...
  upsertCatalog(data) { for(const x of data.items){const k=rowText(x,"name");if(k)state.items[k]={...state.items[k],...x};}for(const x of data.prices){const k=rowText(x,"name")||`${rowText(x,"item_code")}:${rowText(x,"uom")}`;if(k)state.prices[k]={...state.prices[k],...x,price_list:data.priceList};}
    // Drop any cached row tagged with a different (or, from before this field existed, no) price
    // list - same reasoning as database.ts's equivalent cleanup: a stale row from a previously
    // configured price list must not still be eligible to win a lookup for this item.
  ...
  searchCatalog(query,warehouse,priceList) { const q=query.trim().toLowerCase();if(!q)return[];const barcode=resolveBarcodeRow(query);if(barcode){const code=rowText(barcode,"parent");return code&&state.items[code]?[catalogResult(code,query,warehouse,priceList)]:[];}const codes=Object.keys(state.items).filter(code=>{const x=state.items[code];return !x.disabled&&x.is_sales_item!==false&&(code.toLowerCase().includes(q)||rowText(x,"item_name").toLowerCase().includes(q));}).slice(0,50);return codes.map(code=>catalogResult(code,query,warehouse,priceList)); },
  lookupCatalog(query,warehouse,priceList) { const q=query.trim();const barcodeParent=rowText(resolveBarcodeRow(q)??{},"parent");const exactCode=state.items[q]?q:Object.keys(state.items).find((code)=>code.toLowerCase()===q.toLowerCase())||barcodeParent;if(exactCode){const exact=catalogResult(exactCode,query,warehouse,priceList);return{exact,results:[]};}return{exact:null,results:mobileDatabase.searchCatalog(query,warehouse,priceList)}; },
...
function catalogResult(code: string, query: string, warehouse: string, priceList: string): CatalogSearchResult {
  const item=state.items[code]??{};const indexedBarcode=resolveBarcodeRow(query);const barcode=indexedBarcode&&rowText(indexedBarcode,"parent")===code?indexedBarcode:undefined;const uom=rowText(barcode??{},"uom")||rowText(item,"stock_uom");const conversion=Number(conversionIndex.get(`${code}:${uom}`)?.conversion_factor??1)||1;
  ...
  // introduced, before the first post-update sync).
  const allPrices=pricesByItem.get(code)??[];const prices=priceList?allPrices.filter(x=>rowText(x,"price_list")===priceList):allPrices;const direct=prices.find(x=>rowText(x,"uom")===uom);const base=prices.find(x=>rowText(x,"uom")===rowText(item,"stock_uom"));const selected=direct??base;const rate=typeof selected?.price_list_rate==="number"?selected.price_list_rate:null;const stock=state.stock[`${code}:${warehouse}`];const fbr=state.fbr[code];
