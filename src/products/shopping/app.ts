...
import { bottomNavigation, emptyState, escapeHtml as esc, formatMoney, icon, loadingSkeleton, statusBadge, type UiTone } from "../shared/ui";
...
let quotePromise: Promise<void> | null = null;
let checkoutState = { address: "", delivery: "", payment: "", instructions: "" };
...
async function loadProducts() {
  const request = ++productSearchSequence;
...
function nav() {
  ...
  return bottomNavigation("Shopping navigation",active as "home"|"categories"|"cart"|"orders"|"account",[
    {id:"home",label:"Home",icon:icon("home")},{id:"categories",label:"Categories",icon:icon("grid")},
    {id:"cart",label:"Cart",icon:icon("cart"),badge:cartCount()},{id:"orders",label:"Orders",icon:icon("list")},{id:"account",label:"Account",icon:icon("user")}
  ]);
}
...
function productCards(list: Row[]) {
  ...
  return `<div class="product-grid">${sorted.map((product) => {
    ...
    return `<article class="product-card ${product.available===false?"unavailable":""}" data-product="${esc(code)}"><div class="product-image">${src ? `<img src="${esc(src)}" alt="${esc(field(product,"item_name","name"))}" loading="lazy">` : `<div class="image-fallback">${esc(field(product,"item_name","name").slice(0,2))}</div>`}${mrp>rate?`<span class="offer-badge">${icon("tag", 10)} Offer</span>`:""}</div><div class="product-copy"><small>${esc(field(product,"brand","category"))}</small><h3>${esc(field(product,"item_name","name"))}</h3><div class="product-price"><b>${money(rate)}</b>${mrp>rate?`<del>${money(mrp)}</del>`:""}</div><span class="availability ${product.available===false?"out":""}">${product.available===false?"Out of stock":"Available"}</span>${line?`<div class="quantity card-quantity"><button data-product-quantity="${esc(line.id)}" data-delta="-1" aria-label="Decrease quantity">${icon("minus", 16)}</button><b>${line.quantity}</b><button data-product-quantity="${esc(line.id)}" data-delta="1" aria-label="Increase quantity">${icon("plus", 16)}</button></div>`:`<button class="primary add" data-item="${esc(code)}" ${product.available===false?"disabled":""}>${product.available===false?"Unavailable":"Add to cart"}</button>`}</div></article>`;
  }).join("") || emptyState(icon("search", 48),"No products found","Try another search, category or filter.")}</div>`;
}
...
function checkoutView() {
  ...
  const selectedDelivery=checkoutState.delivery||field(delivery[0]||{},"name"),selectedPayment=checkoutState.payment||field(payments[0]||{},"name");
  return `<section class="page checkout-page"><div class="page-heading"><div><button class="back-button" data-screen="cart">${icon("chevron-left", 16)} Cart</button><p class="eyebrow">Secure checkout</p><h1>Complete your order</h1></div></div><div class="checkout-steps"><span class="complete"><b>1</b>Address</span><span class="active"><b>2</b>Delivery</span><span><b>3</b>Payment</span><span><b>4</b>Review</span><span><b>5</b>Done</span></div>${notice?`<div class="notice ${noticeTone}">${esc(notice)}</div>`:""}<div class="checkout-layout"><form id="place" class="form-card"><section><div class="form-heading"><span>1</span><div><h2>Address</h2><p>Choose a saved address or use pickup.</p></div></div>${addresses.length?`<label>Saved address<select id="address"><option value="">No address needed for pickup</option>${addresses.map((item)=>{const value=field(item,"name");return `<option value="${esc(value)}" ${value===checkoutState.address?"selected":""}>${esc(field(item,"label","address_title","name"))}</option>`}).join("")}</select></label>`:`<input id="address" type="hidden" value="${esc(checkoutState.address)}"><p class="inline-info">No delivery address is available. Store Pickup remains available.</p>`}<button type="button" class="text-action" data-address-edit>Add or edit address in your account</button></section><section><div class="form-heading"><span>2</span><div><h2>Delivery or pickup</h2><p>Availability is confirmed for your selected branch.</p></div></div><label>Fulfillment<select id="delivery" required>${delivery.map((item)=>{const value=field(item,"name");return `<option value="${esc(value)}" ${value===selectedDelivery?"selected":""}>${esc(field(item,"label","name"))}</option>`}).join("")}</select></label><label>Delivery instructions<textarea id="instructions" rows="2" placeholder="Optional instructions for the store">${esc(checkoutState.instructions)}</textarea></label></section><section><div class="form-heading"><span>3</span><div><h2>Payment</h2><p>Only currently supported methods are shown.</p></div></div><label>Payment method<select id="payment" required>${payments.map((item)=>{const value=field(item,"name");return `<option value="${esc(value)}" ${value===selectedPayment?"selected":""}>${esc(field(item,"label","name"))}</option>`}).join("")}</select></label><p class="inline-info">Cash on Delivery / Pickup payment is collected according to the selected method.</p></section><button id="refresh-quote" type="button" class="secondary wide" ${placing||quotePromise?"disabled":""}>${quotePromise?"Confirming…":`${icon("refresh", 18)} Confirm stock and totals`}</button></form><aside class="summary checkout-summary"><h2>Review</h2><div class="mini-items">${cart.lines.map((line)=>`<div><span>${line.quantity}× ${esc(line.itemName)}</span><b>${money(line.quantity*line.displayedRate)}</b></div>`).join("")}</div>${quote?`<div class="totals"><p><span>Subtotal</span><b>${money(quote.subtotal,quote.currency)}</b></p><p><span>Discount</span><b>− ${money(quote.discount,quote.currency)}</b></p><p><span>Tax</span><b>${money(quote.taxes,quote.currency)}</b></p><p><span>Fulfillment</span><b>${money(quote.deliveryCharge,quote.currency)}</b></p><p class="grand"><span>Total</span><b>${money(quote.grandTotal,quote.currency)}</b></p></div><button form="place" class="primary wide" ${placing||quotePromise?"disabled":""}>${placing?"Placing order safely…":"Place order"}</button><small>A stable request ID prevents duplicate submission.</small>`:`<div class="quote-prompt"><span>${icon("refresh", 24)}</span><p>Confirm totals to validate current prices and availability before placing the order.</p></div>`}</aside></div></section>`;
}

function ordersView() {
  return `<section class="page orders-page"><div class="page-heading"><div><p class="eyebrow">Your purchases</p><h1>Orders</h1><p>Track pickup and delivery progress in customer-friendly terms.</p></div><button data-screen="catalog">Shop again</button></div>${notice?`<div class="notice ${noticeTone}">${esc(notice)}</div>`:""}<div class="order-list">${orderHistory.map((order)=>{const raw=field(order,"delivery_status","status"),friendly=friendlyStatus(raw);return `<button class="order-card" data-order="${esc(field(order,"name","order"))}"><div><span class="order-icon">${icon("list", 20)}</span><span><strong>${esc(field(order,"name","order"))}</strong><small>${esc(field(order,"transaction_date","creation","date"))}</small><small>${esc(field(order,"item_summary")||"Tap to view order details")}</small></span></div><div>${statusBadge(friendly,statusTone(raw))}<b>${money(field(order,"grand_total"),field(order,"currency")||undefined)}</b><small>${esc(field(order,"delivery_method")||"Store Pickup / Delivery")}</small></div><span>View ${icon("chevron-right", 14)}</span></button>`}).join("")||emptyState(icon("list", 48),"No orders yet","Completed checkouts and their tracking status appear here.",{label:"Start shopping",attribute:'data-screen="catalog"'})}</div></section>`;
}
...
function orderView(){
  ...
  return `<section class="page order-detail-page"><button class="back-button" data-screen="orders">${icon("chevron-left", 16)} All orders</button><div class="order-detail-head"><div><p class="eyebrow">Order tracking</p><h1>${esc(field(order,"name","order"))}</h1><p>Placed ${esc(field(order,"transaction_date","creation","date"))}</p></div>${statusBadge(friendly,statusTone(raw))}</div><div class="tracking-card"><div class="tracking-timeline">${steps.map((step,index)=>`<div class="${index<active?"complete":index===active?"active":""}"><span>${index<active?icon("check", 16):index+1}</span><div><b>${step}</b><small>${index<=active?"Status updated":"Waiting"}</small></div></div>`).join("")}</div><div class="tracking-summary"><h2>Order summary</h2><p><span>Fulfillment</span><b>${esc(field(order,"delivery_method")||"Store Pickup")}</b></p><p><span>Items</span><b>${esc(field(order,"item_summary","total_qty")||"Order items")}</b></p><p class="grand"><span>Total</span><b>${money(field(order,"grand_total"),field(order,"currency")||undefined)}</b></p><button id="reorder" class="primary wide">Reorder available items</button></div></div></section>`
}
...
function bind() {
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-category]").forEach((button) => button.onclick = async () => { category=button.dataset.category || "";search="";loading="Loading products";render();try{await loadProducts();}catch(error){showNotice(error instanceof Error?error.message:"Unable to load category","danger")}finally{loading="";render("catalog")} });
  ...
  const submitSearch=async(query:string)=>{search=query;document.querySelector("#search")?.setAttribute("aria-busy","true");try{const applied=await loadProducts();if(!applied)return;showNotice(products.length?`${products.length} products found`:"No matching products",products.length?"info":"warning");render("catalog")}catch(error){showNotice(error instanceof Error?error.message:"Search failed","danger");render("catalog")}};
  ...
  document.querySelector<HTMLInputElement>("#query")?.addEventListener("input",(event)=>{window.clearTimeout(searchTimer);const value=(event.target as HTMLInputElement).value;search=value;if(value.length===0||value.length>=2)searchTimer=window.setTimeout(()=>void submitSearch(value),300)});
  ...
  document.querySelectorAll<HTMLInputElement>("[data-cart-input]").forEach((input)=>{const apply=()=>{const line=cart.lines.find((item)=>item.id===input.dataset.cartInput);if(!line)return;const quantity=Number(input.value);if(!Number.isFinite(quantity)||quantity<=0){input.value=String(line.quantity);showNotice("Enter a quantity greater than zero.","warning");return}saveCart(setCartQuantity(cart,line.id,quantity));showNotice("Cart quantity updated.","success");render("cart")};input.onchange=apply;input.onkeydown=(event)=>{if(event.key==="Enter"){event.preventDefault();apply()}}});
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-signup]").forEach((button)=>button.addEventListener("click",()=>{if(isNative)void Browser.open({url:signupUrl});else globalThis.open(signupUrl,"_blank","noopener");}));
  ...
  for(const id of ["address","delivery","payment","instructions"]){const control=document.querySelector<HTMLInputElement|HTMLSelectElement|HTMLTextAreaElement>(`#${id}`);control?.addEventListener("change",()=>{const before=`${checkoutState.address}|${checkoutState.delivery}`;readCheckoutState();if(before!==`${checkoutState.address}|${checkoutState.delivery}`){quote=null;showNotice("Fulfillment changed. Confirm stock and totals again.","info");render("checkout")}});control?.addEventListener("input",()=>readCheckoutState())}
...
async function changeStore() {
  if (cart.lines.length && !confirm("Changing store clears this store's local cart and signs you out. Continue?")) return;
...
function readCheckoutState(){checkoutState={address:document.querySelector<HTMLInputElement|HTMLSelectElement>("#address")?.value??checkoutState.address,delivery:document.querySelector<HTMLSelectElement>("#delivery")?.value??checkoutState.delivery,payment:document.querySelector<HTMLSelectElement>("#payment")?.value??checkoutState.payment,instructions:document.querySelector<HTMLTextAreaElement>("#instructions")?.value??checkoutState.instructions}}

async function refreshQuote() {
  if(quotePromise)return quotePromise;
  readCheckoutState();
  quotePromise=Promise.resolve().then(async()=>{render("checkout");try{quote=await body(await api!.quoteCart(cart,checkoutState.delivery,checkoutState.address)) as unknown as ShoppingQuote;showNotice("Prices, stock and totals confirmed.","success")}catch(error){showNotice(error instanceof Error?error.message:"Unable to refresh totals","danger")}}).finally(()=>{quotePromise=null;if(screen==="checkout")render("checkout")});
  return quotePromise;
}
...
async function placeOrder(event: SubmitEvent) {
  ...
  try {
    ...
    const input={requestId:attempt.requestId,quoteToken:quote.quoteToken,addressName:checkoutState.address,deliveryMethod:checkoutState.delivery,paymentMethod:checkoutState.payment};
    ...
    localStorage.removeItem(checkoutKey); saveCart(emptyCart()); quote=null;checkoutState={address:"",delivery:"",payment:"",instructions:""}; showNotice(`Order ${field(result,"sales_order","order","name")} placed successfully.`,"success");
