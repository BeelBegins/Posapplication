...
function documentView(): string {
  ...
  return `<main><header class="top"><button id="back" class="secondary">${icon("chevron-left", 20)} Back</button><span class="${navigator.onLine ? "online" : "offline"}">${navigator.onLine ? "Online" : "Offline"}</span></header><section class="page"><div class="doc-head"><p class="eyebrow">${isPr ? "Purchase Receipt draft" : "Stock Transfer Note"}</p><h1>${esc(text(current, "name"))}</h1><small>${esc(text(current, "branch"))}${text(current, "supplier") ? ` · ${esc(text(current, "supplier"))}` : ""}</small>${isPr ? `<div class="notice">Verify quantities here. Purchase Manager will submit the final PR.</div>` : ""}</div>${notice ? `<div class="notice ${noticeTone}">${esc(notice)}</div>` : ""}<div class="line-list">${rows}</div><label>Remarks<textarea id="remarks" rows="3" placeholder="Optional note"></textarea></label><div class="actions"><button id="scan" class="secondary">${icon("search", 18)} Scan item</button><button id="save-draft" class="secondary">Save on device</button><button id="submit" class="primary" ${busy ? "disabled" : ""}>${busy || (isPr ? "Verify PR" : "Acknowledge STN")}</button></div></section></main>`;
}
...
async function submitCurrent(): Promise<void> {
  ...
  try { await payload(await api!.submitReceiving(request)); const map = draftMap(); delete map[keyFor(current)]; saveRows(draftsKey, map); noticeText(text(current, "source_type") === "Purchase Receipt" ? "PR verified. Purchase Manager must submit the final PR." : "Receiving completed successfully.", "success"); current = null; await refreshPending(); } catch (error) { noticeText(error instanceof Error ? error.message : "Could not submit receiving.", "danger"); render(); } finally { busy = ""; }
}
