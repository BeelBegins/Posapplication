...
The Restaurant facade targets the deferred Phase 2 contract documented in [restaurant-phase2.md](restaurant-phase2.md). The Sales facade uses the restricted, implemented `aimatic.mobile_sales.api` contract documented in [mobile-sales-phase3.md](mobile-sales-phase3.md). Shopping uses the implemented `aimatic.shopping.api` allow-list for public configuration/catalogue, customer registration/account, signed cart quotes, idempotent Sales Order placement, and customer-filtered order history. It does not use generic Resource APIs.
